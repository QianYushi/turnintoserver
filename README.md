...
- `推送低电量通知`：电池模式运行时，低于 50% 和 20% 会通过 Messages 发送 iMessage 提醒。
- `开机自动启动`：登录后自动打开 app。
- `关于应用`：查看版本、检查更新、设置 iMessage 收件地址并测试发送。
- `退出`：退出前恢复系统默认睡眠行为。
...
低电量 iMessage 通知需要先在“关于应用”里填写收件手机号或 Apple ID 邮箱，并点击“测试发送”。首次发送时，macOS 会询问是否允许 `turnintoserver` 控制 Messages，请选择允许。这个功能依赖本机 Messages 已登录可用的 iMessage 账号。
